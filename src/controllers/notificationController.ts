...
import { noContentResponse, successResponse } from "../utils/responseFormatter";
...
export const getUserNotifications = asyncHandler(
	async (req: Request, res: Response) => {
		...
		if (read !== undefined) {
			...
			notifications = await notificationRepository.findByType(
				userId,
				type as string,
			);
		} else {
...
export const cleanupOldNotifications = asyncHandler(
	async (req: Request, res: Response) => {
		...
		return successResponse(
			res,
			null,
			`Notifications older than ${days} days cleaned up`,
		);
	},
);
