 ...
 */
export const executeBatchOperations = asyncHandler(
	async (req: Request, res: Response) => {
		const documentId = req.params.documentId as string;
		const { operations, transactionId: clientTransactionId } =
...
async function executeSaveContentOperation(
	...
	const updated = await documentRepository.updateContent(
		documentId,
		content,
...
async function executeUpdateMetadataOperation(
	...
	if (payload.defaultFont !== undefined)
		updateData.defaultFont = payload.defaultFont;
	if (payload.defaultFontSize !== undefined)
		updateData.defaultFontSize = payload.defaultFontSize;
