...
const saveContentPayloadSchema = Joi.object({
	content: Joi.string().required().allow("", null).messages({
		"any.required": "Content is required for save-content operation",
	}),
	contentChecksum: Joi.string().optional().allow("", null).messages({
...
const batchOperationSchema = Joi.object({
	operationType: Joi.string()
		...
		.messages({
			"any.only":
				"operationType must be one of: save-content, update-metadata, create-checkpoint",
