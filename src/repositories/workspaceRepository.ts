...
export class WorkspaceRepository {
   ...
   */
  async create(workspaceData: Omit<Workspace, 'workspaceId' | 'createdAt' | 'updatedAt'>): Promise<Workspace> {
    const now = new Date();
   ...
   * Update workspace
   */
  async update(workspaceId: string, updates: Partial<Omit<Workspace, 'workspaceId' | 'createdAt'>>): Promise<Workspace> {
    const updateData = {
      ...updates,
      updatedAt: new Date(),
    };
