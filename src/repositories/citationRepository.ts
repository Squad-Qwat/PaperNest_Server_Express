...
export class CitationRepository {
   ...
   */
  async create(citationData: Omit<Citation, 'citationId' | 'createdAt' | 'updatedAt'>): Promise<Citation> {
    const now = new Date();
   ...
   */
  async search(documentId: string, searchTerm: string, limit: number = 10): Promise<Citation[]> {
   ...
   * Update citation
   */
  async update(citationId: string, updates: Partial<Omit<Citation, 'citationId' | 'createdAt'>>): Promise<Citation> {
    const updateData = {
      ...updates,
      updatedAt: new Date(),
    };
