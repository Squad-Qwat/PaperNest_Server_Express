...
export class CommentRepository {
   ...
   */
  async create(commentData: Omit<Comment, 'commentId' | 'createdAt' | 'updatedAt'>): Promise<Comment> {
    const now = new Date();
   ...
   * Update comment
   */
  async update(commentId: string, updates: Partial<Omit<Comment, 'commentId' | 'createdAt'>>): Promise<Comment> {
    const updateData = {
      ...updates,
      updatedAt: new Date(),
    };
