...
export class UserWorkspaceRepository {
   ...
   */
  async create(data: Omit<UserWorkspace, 'userWorkspaceId' | 'createdAt' | 'updatedAt'>): Promise<UserWorkspace> {
    const now = new Date();
   ...
   */
  async findByUserAndWorkspace(userId: string, workspaceId: string): Promise<UserWorkspace | null> {
   ...
   */
  async findWorkspacesByUser(userId: string, status?: string): Promise<UserWorkspace[]> {
   ...
   */
  async findMembersByWorkspace(workspaceId: string, status?: string): Promise<UserWorkspace[]> {
   ...
   */
  async update(userWorkspaceId: string, updates: Partial<Omit<UserWorkspace, 'userWorkspaceId' | 'createdAt'>>): Promise<UserWorkspace> {
    const updateData = {
      ...updates,
      updatedAt: new Date(),
    };
   ...
   */
  async updateInvitationStatus(userWorkspaceId: string, status: string): Promise<UserWorkspace> {
   ...
   * Update member role
   */
  async updateRole(userWorkspaceId: string, role: string): Promise<UserWorkspace> {
  ...
  async deleteByUserAndWorkspace(userId: string, workspaceId: string): Promise<void> {
    const userWorkspace = await this.findByUserAndWorkspace(userId, workspaceId);
  ...
  async hasAccess(userId: string, workspaceId: string): Promise<boolean> {
    const userWorkspace = await this.findByUserAndWorkspace(userId, workspaceId);
    return userWorkspace !== null && userWorkspace.invitationStatus === INVITATION_STATUS.ACCEPTED;
  }
