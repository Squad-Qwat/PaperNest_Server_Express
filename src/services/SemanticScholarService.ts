...
class SemanticScholarService {
	...
	async searchPapers(
		...
		try {
			logger.info(
				`[SemanticScholarService] Searching for papers: "${query}" (limit: ${limit})`,
			);
			...
			logger.error(
				"[SemanticScholarService] Search failed:",
				error.response?.data || error.message,
			);
			throw new Error(
				`Semantic Scholar Search Error: ${error.response?.data?.message || error.message}`,
			);
		}
	}
