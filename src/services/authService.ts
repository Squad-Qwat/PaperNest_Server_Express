...
export const register = async (data: RegisterData): Promise<AuthResponse> => {
  try {
    if (await userRepository.emailExists(data.email)) throw new Error('Email already exists');
    if (await userRepository.usernameExists(data.username)) throw new Error('Username already exists');
...
export const finalizeRegistration = async (firebaseToken: string): Promise<AuthResponse> => {
  try {
    ...
    if (existingUser) {
      ...
      return {
        user: existingUser,
        token: generateToken({ userId: existingUser.userId, email: existingUser.email, role: existingUser.role }),
        refreshToken: generateRefreshToken({ userId: existingUser.userId, email: existingUser.email, role: existingUser.role }),
      };
    }
    ...
    return {
      user,
      token: generateToken({ userId: user.userId, email: user.email, role: user.role }),
      refreshToken: generateRefreshToken({ userId: user.userId, email: user.email, role: user.role }),
    };
  } catch (error: any) {
 ...
 */
export const handleSocialLogin = async (firebaseToken: string, accessToken?: string): Promise<AuthResponse> => {
  try {
    const decodedToken = await auth.verifyIdToken(firebaseToken);
    ...
    let user = await userRepository.findById(uid) || await userRepository.findByLinkedUid(uid);

    if (!user) {
      user = await userRepository.findByEmail(email);
      if (user) {
        logger.info(`Linking new UID ${uid} to existing user ${user.userId} (${email})`);
    ...
    if (!user) {
      return {
        isNewUser: true,
        firebaseToken,
        firebaseData: { uid, email, name: name || '', picture: picture || undefined }
      };
    }
 ...
 */
export const completeSocialRegistration = async (data: CompleteSocialRegistrationData): Promise<AuthResponse> => {
  try {
    ...
    if (existing) throw new Error('An account with this email already exists. Please login instead.');
...
export const refreshAccessToken = async (refreshToken: string): Promise<{ token: string }> => {
  ...
  const user = await userRepository.findById(decoded.userId) || await userRepository.findByLinkedUid(decoded.userId);
  if (!user) throw new Error(ERROR_MESSAGES.USER_NOT_FOUND);
  return { token: generateToken({ userId: user.userId, email: user.email, role: user.role }) };
};

export const verifyFirebaseToken = async (token: string): Promise<User> => {
  const decodedToken = await auth.verifyIdToken(token);
  const user = await userRepository.findById(decodedToken.uid) || await userRepository.findByLinkedUid(decodedToken.uid);
  if (!user) throw new Error(ERROR_MESSAGES.USER_NOT_FOUND);
  return user;
};
