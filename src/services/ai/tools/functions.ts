...
const parseApplyDiffPairs = (
	args: any,
): { pairs?: DiffPair[]; error?: string } => {
	...
	for (let i = 0; i < searchBlock.length; i++) {
		...
		if (search.length === 0) {
			return {
				error: `Error: apply_diff_edit searchBlock[${i}] cannot be empty.`,
			};
		}
...
const findAtomicDiffMatches = (
	...
	if (
		pairs.length === 1 &&
		candidateMatchesByPair[0].length > 1 &&
		!isDeletionByPair[0]
	) {
		return {
	...
	const backtrack = (pairIdx: number, minStart: number) => {
		...
		if (isDeletionByPair[pairIdx]) {
			...
			const validCandidates = candidateMatchesByPair[pairIdx].filter(
				(c) => c.from >= minStart,
			);
...
const applyMatchesToText = (docText: string, matches: DiffMatch[]): string => {
	...
	for (const match of sortedDesc) {
		result =
			result.slice(0, match.from) + match.replace + result.slice(match.to);
	}
...
const findLatexBibliographyStart = (
	text: string,
	limitExclusive?: number,
): number | null => {
	const limit =
		typeof limitExclusive === "number" ? limitExclusive : text.length;
	const patterns = [
		...
		switch (toolName) {
			case "read_document": {
				...
				if (fromLine !== undefined) {
					...
					return (
						`[Document Slice: Lines ${startLine} to ${endLine}]\nTotal Lines: ${doc.lines}\n\n` +
			...
			case "get_sections": {
				...
				const sections: Array<{ text: string; level: number; line: number }> =
					[];
				let match: RegExpExecArray | null = sectionRegex.exec(docText);
			...
			case "get_document_stats": {
				const docText = view.state.doc.toString();
				const words =
					docText.trim().length === 0 ? 0 : docText.trim().split(/\s+/).length;
			...
			case "search_text_lines": {
				...
				output +=
					`\n` +
					results
						.map((r) => `${String(r.line).padStart(4, " ")} | ${r.text}`)
						.join("\n");
			...
			case "replace_lines": {
				...
				if (stage) {
					...
					const anchored = createAnchoredReplacePair(
						original,
						fromPos,
						toPos,
						newContent,
			...
			case "insert_content": {
				...
				const bibliographyLimit =
					endDocumentIndex >= 0 ? endDocumentIndex : undefined;
