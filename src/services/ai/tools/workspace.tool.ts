...
export const searchWorkspaceDocumentsTool = (workspaceId: string) => {
	return tool(
		...
		},
		{
			...
			description:
				"Search for documents in the current workspace by their title. Use this when the user references another document by title or asks to search for documents in their workspace.",
			schema: z.object({
				query: z
					.string()
					.describe("The document title query or keyword to search for"),
			}),
		},
	);
};
...
export const readWorkspaceDocumentByIdTool = (workspaceId: string) => {
	return tool(
		async ({ documentId }) => {
			...
			try {
				...
				if (content.length > maxSafeLength) {
					...
					return (
						`[Document Title: ${document.title}] (ID: ${document.documentId})\n\n` +
		...
		},
		{
			...
			description:
				"Read the content of a specific document in the workspace by its Document ID. Make sure to retrieve the Document ID first using search_workspace_documents.",
			schema: z.object({
...
export const searchDocumentLinesBackendTool = (workspaceId: string) => {
	return tool(
		...
		},
		{
			...
			description:
				"Search for specific text inside a LaTeX document by line content on the server database. Use this in the AI Dashboard to locate lines of interest without opening the client editor.",
			schema: z.object({
				documentId: z
					.string()
					.describe("The exact ID of the document to search"),
	...
	return tool(
		async ({ documentId, fromLine, toLine }) => {
			try {
				...
				const end = Math.min(
					lines.length,
					toLine || Math.min(lines.length, start + 49),
				);
				...
				const formatted = slice
					.map(
						(lineText: string, idx: number) =>
							`Line ${start + idx}: ${lineText}`,
					)
					.join("\n");
		...
		},
		{
			...
			description:
				"Read a specific range of lines from a document's LaTeX source code directly from the server database. Use this in the AI Dashboard to inspect parts of documents without opening the editor.",
			schema: z.object({
				documentId: z.string().describe("The exact ID of the document to read"),
				fromLine: z
					.number()
...
export const searchWorkspacePdfsRagTool = (workspaceId: string) => {
	return tool(
		async ({ query, maxChunks = 5 }) => {
			...
			try {
				const results = await ragService.searchWorkspace(
					workspaceId,
					query,
					maxChunks,
				);
		...
		},
		{
			...
			description:
				"Search semantically through all uploaded PDF documents and references attached within the current workspace. Use this when the user asks general questions about papers, research papers, or background facts across their workspace documents.",
			schema: z.object({
				query: z
					.string()
					.describe(
						"The semantic search query or keywords to find in workspace PDFs",
					),
				maxChunks: z
					.number()
					.optional()
					.default(5)
					.describe("Maximum number of snippets to retrieve"),
			}),
		},
	);
};

export const getActiveToolsForState = (state: {
	documentId?: string;
	workspaceId?: string;
}): any[] => {
