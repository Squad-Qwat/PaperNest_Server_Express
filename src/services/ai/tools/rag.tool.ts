...
export const createRAGTool = (documentId: string) => {
    return tool(
        async ({ query, maxChunks = 5 }) => {
            console.log(`[RAGTool] Searching for: "${query}" in document ${documentId}`)
        ...
        },
        {
            ...
            description: "Search through the content of all uploaded PDF files attached to this document. Use this when the user asks questions about existing research, data, or content that might be in their attached PDF references.",
            schema: z.object({
                query: z.string().describe("The search query or keywords to look for in the PDFs"),
                maxChunks: z.number().optional().default(5).describe("Maximum number of relevant excerpts to return")
            })
        }
    )
}
