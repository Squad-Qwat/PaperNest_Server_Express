...
test('apply_diff_edit stage mode includes batch metadata', async () => {
	...
	assert.equal(result.type, 'staged_change')
	assert.equal(result.description, 'Apply diff edit (batch 2 items)')
	assert.deepEqual(result.searchBlock, ['X1\nY1', 'X2\nY2'])
	assert.deepEqual(result.replaceBlock, ['R1', 'R2'])
	assert.equal(result.modified, 'R1\n---\nR2')
})
