...
6. Each step must use ONE tool: `read_document`, `get_sections`, `search_text_lines`, `replace_lines`, `apply_diff_edit`, `insert_content`, `compile_latex`.
7. **GREETINGS/CHAT**: If the task is purely conversational, set `tool` to `null` and provide a friendly response in `reasoning`.
