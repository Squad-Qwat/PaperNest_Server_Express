...
export const reflectorNode = async (state: AgentStateType) => {
	...
	if (
		...
		console.warn(
			"[Reflector] Active step has invalid description, marking as failed",
		);
		if (activeIndex >= 0) {
	...
	console.log(
		`[Reflector] Active step: ${activeStep?.id} (tool: ${activeStep?.tool}, status: ${activeStep?.status})`,
	);
	...
	console.log("[Reflector] lastToolResults available:", {
		...
		results: state.lastToolResults?.map((r) => ({
			name: r.name,
			hasResult: !!r.result,
		})),
	});
	...
	const reflectorPrompt = prompts.reflector
		...
		.replace(
			"{acceptance_criteria}",
			activeStep.acceptanceCriteria || "Step executed without error",
		)
	...
	if (noExecutionDetected) {
		...
		const shouldLightReplan =
			consecutiveNoExecutionCycles >= 2 && !hasMeaningfulText;
	...
	try {
		...
		console.log(
			`[Reflector] Verdict for step "${activeStep?.description}": ${verdict.slice(0, 80)}`,
		);
		...
		if (activeIndex !== -1 && plan[activeIndex].status === "active") {
			...
			console.log(
				`[Reflector] Step ${activeStep?.id}: active → failed (due to LLM error)`,
			);
		}
		needsReplanning = true;
		confidence = 0.2;
