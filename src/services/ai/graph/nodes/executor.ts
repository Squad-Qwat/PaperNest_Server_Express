...
export const executorNode = async (state: AgentStateType) => {
	...
	if (
		...
		console.warn(
			"[Executor] Current step invalid or missing description:",
			currentStep,
		);
		...
		const failedPlan = state.plan.map((s) => {
			if (s.id === currentStep?.id) {
				console.warn(
					`[Executor] Marking step ${s.id} as FAILED due to missing description`,
				);
	...
	try {
		...
		console.error(
			"[Executor] invoke failed with full history, retrying with minimal context:",
			{
	...
	const contentPreviewText =
		typeof response.content === "string"
			...
			: Array.isArray(response.content)
				? `[Array of ${response.content.length}]`
				: "non-string";
