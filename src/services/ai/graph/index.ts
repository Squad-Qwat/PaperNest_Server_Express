    ...
    try {
        ...
        for await (const update of await graph.stream(initialState, config)) {
            ...
            for (const [nodeName, nodeOutput] of entries) {
                ...
                // Stream Plan Updates
                if ((nodeName === ROUTES.PLANNER || nodeName === ROUTES.REFLECTOR || nodeName === ROUTES.EXECUTOR) && output.plan) {
                ...
                if (nodeName === ROUTES.EXECUTOR && output.messages) {
                    ...
                    const newAiMsg = [...output.messages].reverse().find(
                        (m: any) => (typeof m._getType === 'function' ? m._getType() : m.type) === 'ai'
                    )
                    if (newAiMsg) {
                        let textContent = ''
        ...
        const { inputTokens = 0, outputTokens = 0, reasoningTokens = 0 } = stateValues
        ...
        console.log(`\x1b[32m[AI Stats] Input Tokens     : ${inputTokens.toLocaleString()}\x1b[0m`)
        console.log(`\x1b[32m[AI Stats] Output Tokens    : ${outputTokens.toLocaleString()}\x1b[0m`)
        console.log(`\x1b[32m[AI Stats] Thinking Tokens  : ${reasoningTokens.toLocaleString()}\x1b[0m`)
        console.log(`\x1b[32m[AI Stats] Est. Cost (IDR)  : Rp ${totalCostIdr.toFixed(2)}\x1b[0m`)
