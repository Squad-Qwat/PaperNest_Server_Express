...
export class GeminiKeyRotator {
	...
	private constructor() {
		const rawKeys =
			process.env.GOOGLE_API_KEYS || process.env.GEMINI_API_KEYS || "";
		...
		if (this.keys.length === 0) {
			throw new Error(
				"No Gemini or Google API keys configured in environment variables",
			);
		}
	}
	...
	public markAsCooldown(keyValue: string): void {
		...
		if (key) {
			...
			console.warn(
				`[GeminiKeyRotator] Key marked as COOLDOWN: index ${this.keys.indexOf(key)}`,
			);
		}
		this.currentPointer = (this.currentPointer + 1) % this.keys.length;
	}
	...
	private cleanupCooldowns(): void {
		...
		for (const key of this.keys) {
			if (key.status === "cooldown" && now > key.cooldownUntil) {
				...
				key.cooldownUntil = 0;
				console.log(
					`[GeminiKeyRotator] Key restored to ACTIVE: index ${this.keys.indexOf(key)}`,
				);
			}
		}
	}
...
export async function generateEmbeddingsWithFailover(
	texts: string[],
): Promise<number[][]> {
	const rotator = GeminiKeyRotator.getInstance();
