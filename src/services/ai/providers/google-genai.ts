...
export class GoogleGenAIProvider implements AIProvider {
	...
	private supportsThinkingConfig(modelName: string): boolean {
		...
		return normalized.includes('gemini-2.5') || normalized.includes('gemini-3') || normalized.includes('gemma-4')
	}
	...
	createModel(config: AIProviderConfig): BaseChatModel {
		...
		const shouldApplyThinking = this.supportsThinkingConfig(modelName) && config.reasoningEnabled
