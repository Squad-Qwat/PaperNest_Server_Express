...
export class RAGRepository {
     ...
     */
    async saveChunks(documentId: string, fileKey: string, chunks: PDFChunk[]): Promise<void> {
        ...
        await batch.commit()
        console.log(`[RAGRepository] Saved ${chunks.length} chunks for document ${documentId}`)
    }
     ...
     */
    async search(documentId: string, query: string, limit: number = 5): Promise<StoredRAGChunk[]> {
        ...
        const results = snapshot.docs
            ...
            .sort((a, b) => {
                const aMatches = searchTerms.filter(t => a.text.toLowerCase().includes(t)).length
                const bMatches = searchTerms.filter(t => b.text.toLowerCase().includes(t)).length
