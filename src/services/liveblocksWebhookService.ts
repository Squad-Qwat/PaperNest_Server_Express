...
import type { RoomCleanupResult } from "../types";
...
export class LiveblocksWebhookService {
	...
	async checkAndCleanupRoom(roomId: string): Promise<RoomCleanupResult> {
		try {
			...
			if (activeUsers.length > 0) {
				logger.info(
					`Room ${roomId} has ${activeUsers.length} active users - keeping room`,
				);
				return { action: "keep_room", activeUsers: activeUsers.length };
			}

			logger.info(
				`Room ${roomId} has 0 active users. Preserving room state to maintain Yjs CRDT history.`,
			);
	...
	async getActiveUsers(roomId: string): Promise<any[]> {
		...
		if (!response.ok) {
			...
			throw new Error(
				`Failed to get active users: ${response.status} ${errorText}`,
			);
		}
